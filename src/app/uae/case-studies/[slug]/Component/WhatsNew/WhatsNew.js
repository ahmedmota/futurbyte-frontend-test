...
const WhatsNew = ({
  ...
  return (
    ...
    >
      <Container className={styles["testionomial-sec-container"]}>
        <div className="d-flex justify-content-between">
          <div>
            <h2 className={`colorBlack font48 fw600 text-left text-md-center`}>
              {heading}
            </h2>
            {description && (
              <p className={styles["case-study-desc"]}>{description}</p>
            )}
          </div>
          {viewAllBtn && (
            <Link
              className={styles["view-all-btn"]}
              href={viewAllBtn.buttonLink ? ORIGINS.UAE + viewAllBtn.buttonLink : "#"}
            >
              {viewAllBtn.buttonLabel}
            </Link>
          )}
        </div>
        {testimonials && testimonials.length === 1 ? (
          <div className={`${styles.card} ${styles.testimonailsCategory}`}>
            ...
            <div className={styles.titmonialLink}>
              {testimonials[0]?.button && (
                <Link
                  ...
                  href={ORIGINS.UAE + testimonials[0]?.buttonLink}
                >
                ...
                <div key={testimonial.id} className={styles.card}>
                  ...
                  <div className={styles.titmonialLink}>
                    {testimonial.button && (
                      <Link
                        className={`d-flex gap-2 align-items-center textdecoration ${styles["learn-more-b"]}`}
                        href={ORIGINS.UAE + testimonial?.buttonLink}
                      >
                        <span className="position-relative">
                          {testimonial?.button}
                        </span>
