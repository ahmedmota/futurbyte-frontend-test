...
export async function generateMetadata({ params }) {
  const pageName = `${params.cat}/${params["sub-cat"]}`
...
export default async function CatPage({ params }) {
  const dynamicPageName = `${params.cat}/${params["sub-cat"]}`

  const data = await getOriginData(ORIGINS.UAE,`/seos?filters[slug][$eq]=${dynamicPageName}`)
