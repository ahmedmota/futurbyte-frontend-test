...
export default async function Partnership() {
  ...
  const getHeader = await getOriginData(ORIGINS.UAE,`/partnership-header?populate=*`);
  const getLetsBePartner = await getOriginData(ORIGINS.UAE,
    `/partnership-lets-be-partner?populate=items.icon`
  );
  const getOurLegacy = await getOriginData(ORIGINS.UAE,
