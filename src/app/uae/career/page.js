...
const page = async () => {
  ...
  const [header, joinTeam, jobs, everyoneDriven, futureByteWorld, joinUs] =
    await Promise.all([
      getOriginData(ORIGINS.UAE,"/career-header?populate=*"),
      getOriginData(ORIGINS.UAE,"/career-join-the-team?populate=*"),
      getOriginData(ORIGINS.UAE,"/career-detail-infos"),
      getOriginData(ORIGINS.UAE,
        "/career-everyone-is-driven?populate=background_image&populate=items.icon"
      ),
      getOriginData(ORIGINS.UAE,"/career-the-futurbyte-world?populate=*"),
      getOriginData(ORIGINS.UAE,"/career-join-the-futurbyte?populate=*"),
    ]);
