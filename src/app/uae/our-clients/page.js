...
export default async function OurClients() {
  ...
  const getHeader = await getOriginData(ORIGINS.UAE,`/our-clients-header?populate=*`);
  const getClients = await getOriginData(ORIGINS.UAE,`/our-clients-list-items?populate=*`);
  const getRecognisedServices = await getOriginData(ORIGINS.UAE,
