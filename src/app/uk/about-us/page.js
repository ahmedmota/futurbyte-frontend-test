...
export default async function AboutPage() {
  ...
  const [header, beginnings, commitments, privileged, getStart, ourTeams] =
    await Promise.all([
      getOriginData(ORIGINS.UK, "/about-header?populate=*"),
      getOriginData(ORIGINS.UK, "/about-our-beginnings?populate=*"),
      getOriginData(ORIGINS.UK, "/about-we-are-recognized?populate=items&populate=items.image"),
      getOriginData(ORIGINS.UK, "/privileged-to-be?populate=logos.logo"),
      getOriginData(ORIGINS.UK, "/lets-get-started?populate=*"),
      getOriginData(ORIGINS.UK, "/about-team?populate=about_team_items.image"),
    ]);
