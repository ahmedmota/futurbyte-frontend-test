...
export default async function Partnership() {
  ...
  const getHeader = await getOriginData(ORIGINS.UK,`/partnership-header?populate=*`);
  const getLetsBePartner = await getOriginData(ORIGINS.UK,
    `/partnership-lets-be-partner?populate=items.icon`
  );
  const getOurLegacy = await getOriginData(ORIGINS.UK,
