...
export default async function Augmentation() {
  const getHeaderApi = await getOriginData(ORIGINS.UK,
    `/resource-augmentation-header?populate=*`
  );
  const getSaveTheTime = await getOriginData(ORIGINS.UK,
    `/resource-augmentation-save-the-time?populate=items.icon`
  );
  const getHowItWorks = await getOriginData(ORIGINS.UK,
    `/resource-augmentation-how-it-work?populate=resource_augmentation_how_it_works_items.image`
  );
  const getResourceAugmentation = await getOriginData(ORIGINS.UK,
  ...
  const getFaqs = await getOriginData(ORIGINS.UK,`/resource-augmentation-fa-q?populate=*`);
