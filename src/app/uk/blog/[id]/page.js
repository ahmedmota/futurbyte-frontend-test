...
export async function generateMetadata({ params: { id } }) {
  ...
  const data = await getOriginData(ORIGINS.UK,`/seos?filters[slug][$eq]=${id}`);
...
export default async function BlogDetailPage({ params }) {
  ...
  const getContentApi = await getOriginData(ORIGINS.UK,
    `/blog-detail-blogs?populate=*&filters[slug][$eq]=${id}`
  );
  const getContentFaqs = await getOriginData(ORIGINS.UK,
  ...
  const getRelatedBlogsApi = await getOriginData(ORIGINS.UK,
  ...
  const getMayLikeBlogsApi = await getOriginData(ORIGINS.UK,
