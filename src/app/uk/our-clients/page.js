...
export default async function OurClients() {
  ...
  const getHeader = await getOriginData(ORIGINS.UK,`/our-clients-header?populate=*`);
  const getClients = await getOriginData(ORIGINS.UK,`/our-clients-list-items?populate=*`);
  const getRecognisedServices = await getOriginData(ORIGINS.UK,
