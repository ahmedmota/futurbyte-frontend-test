...
export async function generateMetadata() {
  ...
  const data = await getOriginData(ORIGINS.UK, `/seos?filters[slug][$eq]=/`);
  return getMetaFromJSON(data?.[0], pageName, ORIGINS.UK, false);
}
...
export default async function Home() {
  ...
  const [
    ...
    getOriginData(ORIGINS.UK, "/brand-reputation?populate=left_logos.logo&populate=right_logos.logo&populate=main_logo&populate=titles"),
    getOriginData(ORIGINS.UK, "/providing-digital-solution?populate=icons.icon"),
    getOriginData(ORIGINS.UK,
    ...
    getOriginData(ORIGINS.UK, "/customer-review?populate=*"),
    getOriginData(ORIGINS.UK,
    ...
    getOriginData(ORIGINS.UK, "/whats-new?populate=whats_new_news&populate=whats_new_news.image"),
    getOriginData(ORIGINS.UK, "/privileged-to-be?populate=logos.logo"),
    getOriginData(ORIGINS.UK,
    ...
    data.homepage_our_digital_solution_buttons.forEach((ds) => {
      if (ds.main_button_label) {
        digitalSolutionsTabContent[ds.main_button_label] = {
          ...
          learnMoreLink: ORIGINS.UK + ds.button_link,
          list: ds.homepage_our_digital_solution_pages_links.map((item) => ({
            link: ORIGINS.UK + item.link,
            label: item.label,
          })),
        };
      }
    ...
    <>
      ...
      {header && (
        <Banner
          ...
          ourWorkLink={ORIGINS.UK + header.buttons[1].link}
          button_link={ORIGINS.UK + header.buttons[0].link}
     ...
     {brandReputations && (
      ...
      {scheduleCall && (
        <div className="postImage">
          <HeroSection
            ...
            buttonLink={ORIGINS.UK + scheduleCall.button_link}
          />
        </div>
      )}
      ...
      {getStart && (
        <GetStartedSection
          ...
          buttonLink={ORIGINS.UK + getStart.button_link}
