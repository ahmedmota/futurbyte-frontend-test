...
export default async function Services() {
  ...
  const bannerResponse = await getOriginData(ORIGINS.UK,"/services-header?populate=*");
  const servicesResponse = await getOriginData(ORIGINS.UK,
    "/services-sections?populate=image&populate=services_services_section_links"
  );
  const letsGetStartedResponse = await getOriginData(ORIGINS.UK,"/let-s-get-started?populate=*");
  const faqResponse = await getOriginData(ORIGINS.UK,"/frequently-ask-question?populate=*");
  const privileged = await getOriginData(ORIGINS.UK,"/privileged-to-be?populate=logos.logo");
  const header = await getOriginData(ORIGINS.UK,
