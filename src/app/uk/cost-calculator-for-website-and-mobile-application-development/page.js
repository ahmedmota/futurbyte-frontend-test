...
export default async function Calculator() {
  ...
  const costCalculatorData = await getOriginData(ORIGINS.UK,
    "/cost-calculator-steps?populate=step_choices.icon"
  );
  const getInstantCostCalculator = await getOriginData(ORIGINS.UK,
    "/cost-calculator-get-instant-cost-calculation"
  );
  const getBenefitsCalculator = await getOriginData(ORIGINS.UK,
    "/cost-calcultor-the-benefit?populate=background_image&populate=items.icon"
  );
  const getInstantAccurateCost = await getOriginData(ORIGINS.UK,
    "/cost-calculator-get-instant-accurate-cost?populate=*"
  );
  const costCalculatorCalculatingApp = await getOriginData(ORIGINS.UK,
