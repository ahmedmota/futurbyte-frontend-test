...
const page = async () => {
  ...
  const [header, joinTeam, jobs, everyoneDriven, futureByteWorld, joinUs] =
    await Promise.all([
      getOriginData(ORIGINS.UK,"/career-header?populate=*"),
      getOriginData(ORIGINS.UK,"/career-join-the-team?populate=*"),
      getOriginData(ORIGINS.UK,"/career-detail-infos"),
      getOriginData(ORIGINS.UK,
        "/career-everyone-is-driven?populate=background_image&populate=items.icon"
      ),
      getOriginData(ORIGINS.UK,"/career-the-futurbyte-world?populate=*"),
      getOriginData(ORIGINS.UK,"/career-join-the-futurbyte?populate=*"),
    ]);
