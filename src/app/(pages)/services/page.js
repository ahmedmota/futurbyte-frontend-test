...
export default async function Services() {
  ...
  const bannerResponse = await getData("/services-header?populate=*");
  const servicesResponse = await getData(
    "/services-sections?populate=image&populate=services_services_section_links"
  );
  const letsGetStartedResponse = await getData("/let-s-get-started?populate=*");
  const faqResponse = await getData("/frequently-ask-question?populate=*");
  const privileged = await getData("/privileged-to-be?populate=logos.logo");
