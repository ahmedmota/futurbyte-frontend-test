import { getData } from "@/lib/api"; // Custom data-fetching function
...
export default async function AboutPage() {
  ...
  const [header, beginnings, commitments, privileged, getStart, ourTeams] =
    await Promise.all([
      getData("/about-header?populate=*"),
      getData("/about-our-beginnings?populate=*"),
      getData("/about-we-are-recognized?populate=items&populate=items.image"),
      getData("/privileged-to-be?populate=logos.logo"),
      getData("/lets-get-started?populate=*"),
      getData("/about-team?populate=about_team_items.image"),
    ]);
