...
export async function generateMetadata() {
  ...
  const data = await getData(`/seos?filters[slug][$eq]=/`);
  return getMetaFromJSON(data?.[0], pageName, "", false);
}
...
export default async function Home() {
  ...
  const [
    ...
    getData("/brand-reputation?populate=left_logos.logo&populate=right_logos.logo&populate=main_logo&populate=titles"),
    getData("/providing-digital-solution?populate=icons.icon"),
    getData(
    ...
    getData("/customer-review?populate=*"),
    getData(
      "/video-testimonial?populate[video_testimonial_collections][populate]=video"
    ),
    getData("/lets-get-started?populate=*"),
    getData("/whats-new?populate=whats_new_news&populate=whats_new_news.image"),
    getData("/privileged-to-be?populate=logos.logo"),
    getData(
    ...
    <>
      ...
      {header && (
        <Banner
          ...
          ourWorkLink={header.buttons[1].link}
          button_link={header.buttons[0].link}
      ...
      {scheduleCall && (
        <div className="postImage">
          <HeroSection
            ...
            buttonLink={scheduleCall.button_link}
          />
        </div>
      )}
