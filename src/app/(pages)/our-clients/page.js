...
export default async function OurClients() {
  ...
  const getHeader = await getData(`/our-clients-header?populate=*`);
  const getClients = await getData(`/our-clients-list-items?populate=*`);
  const getRecognisedServices = await getData(
