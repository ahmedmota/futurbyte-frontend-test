...
export async function generateMetadata({ params: { id } }) {
  ...
  const data = await getData(`/seos?filters[slug][$eq]=${id}`);
...
export default async function BlogDetailPage({ params }) {
  ...
  const getContentApi = await getData(
    `/blog-detail-blogs?populate=*&filters[slug][$eq]=${id}`
  );
  const getContentFaqs = await getData(
  ...
  const getRelatedBlogsApi = await getData(
  ...
  const getMayLikeBlogsApi = await getData(
