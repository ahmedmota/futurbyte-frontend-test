...
export default async function Calculator() {
  ...
  const costCalculatorData = await getOriginData(ORIGINS.US,
    "/cost-calculator-steps?populate=step_choices.icon"
  );
  const getInstantCostCalculator = await getOriginData(ORIGINS.US,
    "/cost-calculator-get-instant-cost-calculation"
  );
  const getBenefitsCalculator = await getOriginData(ORIGINS.US,
    "/cost-calcultor-the-benefit?populate=background_image&populate=items.icon"
  );
  const getInstantAccurateCost = await getOriginData(ORIGINS.US,
    "/cost-calculator-get-instant-accurate-cost?populate=*"
  );
  const costCalculatorCalculatingApp = await getOriginData(ORIGINS.US,
