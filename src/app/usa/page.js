...
export async function generateMetadata() {
  ...
  const data = await getOriginData(ORIGINS.US, `/seos?filters[slug][$eq]=/`);
  return getMetaFromJSON(data?.[0], pageName, ORIGINS.US, false);
}
...
export default async function Home() {
  ...
  const [
    ...
    getOriginData(ORIGINS.US, "/header?populate=*"),
    getOriginData(ORIGINS.US,
    ...
    getOriginData(ORIGINS.US, "/brand-reputation?populate=left_logos.logo&populate=right_logos.logo&populate=main_logo&populate=titles"),
    getOriginData(ORIGINS.US, "/providing-digital-solution?populate=icons.icon"),
    getOriginData(ORIGINS.US,
    ...
    getOriginData(ORIGINS.US, "/customer-review?populate=*"),
    getOriginData(ORIGINS.US,
    ...
    getOriginData(ORIGINS.US, "/whats-new?populate=whats_new_news&populate=whats_new_news.image"),
    ...
    data.homepage_our_digital_solution_buttons.forEach((ds) => {
      if (ds.main_button_label) {
        digitalSolutionsTabContent[ds.main_button_label] = {
          ...
          learnMoreLink: ORIGINS.US + ds.button_link,
          list: ds.homepage_our_digital_solution_pages_links.map((item) => ({
            link: ORIGINS.US + item.link,
            label: item.label,
          })),
        };
      }
    ...
    <>
      ...
      {header && (
        <Banner
          ...
          ourWorkLink={ORIGINS.US + header.buttons[1].link}
          button_link={ORIGINS.US + header.buttons[0].link}
     ...
     {brandReputations && (
      ...
      {scheduleCall && (
        <div className="postImage">
          <HeroSection
            ...
            buttonLink={ORIGINS.US + scheduleCall.button_link}
          />
        </div>
      )}
      ...
      {getStart && (
        <GetStartedSection
          ...
          buttonLink={ORIGINS.US + getStart.button_link}
