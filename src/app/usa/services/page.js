...
export default async function Services() {
  ...
  const bannerResponse = await getOriginData(ORIGINS.US,"/services-header?populate=*");
  const servicesResponse = await getOriginData(ORIGINS.US,
    "/services-sections?populate=image&populate=services_services_section_links"
  );
  const letsGetStartedResponse = await getOriginData(ORIGINS.US,"/let-s-get-started?populate=*");
  const faqResponse = await getOriginData(ORIGINS.US,"/frequently-ask-question?populate=*");
  const privileged = await getOriginData(ORIGINS.US,"/privileged-to-be?populate=logos.logo");
  const header = await getOriginData(ORIGINS.US,
