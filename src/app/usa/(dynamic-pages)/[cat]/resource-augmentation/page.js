...
export default async function Augmentation() {
  const getHeaderApi = await getOriginData(ORIGINS.US,
    `/resource-augmentation-header?populate=*`
  );
  const getSaveTheTime = await getOriginData(ORIGINS.US,
    `/resource-augmentation-save-the-time?populate=items.icon`
  );
  const getHowItWorks = await getOriginData(ORIGINS.US,
    `/resource-augmentation-how-it-work?populate=resource_augmentation_how_it_works_items.image`
  );
  const getResourceAugmentation = await getOriginData(ORIGINS.US,
  ...
  const getFaqs = await getOriginData(ORIGINS.US,`/resource-augmentation-fa-q?populate=*`);
