...
export default async function OurClients() {
  ...
  const getHeader = await getOriginData(ORIGINS.US,`/our-clients-header?populate=*`);
  const getClients = await getOriginData(ORIGINS.US,`/our-clients-list-items?populate=*`);
  const getRecognisedServices = await getOriginData(ORIGINS.US,
