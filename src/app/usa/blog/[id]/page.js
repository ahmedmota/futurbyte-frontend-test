...
export async function generateMetadata({ params: { id } }) {
  ...
  const data = await getOriginData(ORIGINS.US,`/seos?filters[slug][$eq]=${id}`);
...
export default async function BlogDetailPage({ params }) {
  ...
  const getContentApi = await getOriginData(ORIGINS.US,
    `/blog-detail-blogs?populate=*&filters[slug][$eq]=${id}`
  );
  const getContentFaqs = await getOriginData(ORIGINS.US,
  ...
  const getRelatedBlogsApi = await getOriginData(ORIGINS.US,
  ...
  const getMayLikeBlogsApi = await getOriginData(ORIGINS.US,
