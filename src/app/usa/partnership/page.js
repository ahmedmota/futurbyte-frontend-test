...
export default async function Partnership() {
  ...
  const getHeader = await getOriginData(ORIGINS.US,`/partnership-header?populate=*`);
  const getLetsBePartner = await getOriginData(ORIGINS.US,
    `/partnership-lets-be-partner?populate=items.icon`
  );
  const getOurLegacy = await getOriginData(ORIGINS.US,
