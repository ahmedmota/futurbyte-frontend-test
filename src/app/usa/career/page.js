...
const page = async () => {
  ...
  const [header, joinTeam, jobs, everyoneDriven, futureByteWorld, joinUs] =
    await Promise.all([
      getOriginData(ORIGINS.US,"/career-header?populate=*"),
      getOriginData(ORIGINS.US,"/career-join-the-team?populate=*"),
      getOriginData(ORIGINS.US,"/career-detail-infos"),
      getOriginData(ORIGINS.US,
        "/career-everyone-is-driven?populate=background_image&populate=items.icon"
      ),
      getOriginData(ORIGINS.US,"/career-the-futurbyte-world?populate=*"),
      getOriginData(ORIGINS.US,"/career-join-the-futurbyte?populate=*"),
    ]);
