...
const page = async () => {
  ...
  const [header, joinTeam, jobs, everyoneDriven, futureByteWorld, joinUs] =
    await Promise.all([
      getOriginData(ORIGINS.CA,"/career-header?populate=*"),
      getOriginData(ORIGINS.CA,"/career-join-the-team?populate=*"),
      getOriginData(ORIGINS.CA,"/career-detail-infos"),
      getOriginData(ORIGINS.CA,
        "/career-everyone-is-driven?populate=background_image&populate=items.icon"
      ),
      getOriginData(ORIGINS.CA,"/career-the-futurbyte-world?populate=*"),
      getOriginData(ORIGINS.CA,"/career-join-the-futurbyte?populate=*"),
    ]);
