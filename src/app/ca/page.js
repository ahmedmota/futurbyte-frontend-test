...
export async function generateMetadata() {
  ...
  const data = await getOriginData(ORIGINS.CA, `/seos?filters[slug][$eq]=/`);
  return getMetaFromJSON(data?.[0], pageName, ORIGINS.CA, false);
}
...
export default async function Home() {
  ...
  const [
    ...
    getOriginData(ORIGINS.CA, "/header?populate=*"),
    getOriginData(ORIGINS.CA,
    ...
    getOriginData(ORIGINS.CA, "/brand-reputation?populate=left_logos.logo&populate=right_logos.logo&populate=main_logo&populate=titles"),
    getOriginData(ORIGINS.CA, "/providing-digital-solution?populate=icons.icon"),
    getOriginData(ORIGINS.CA,
    ...
    getOriginData(ORIGINS.CA, "/customer-review?populate=*"),
    getOriginData(ORIGINS.CA,
    ...
    getOriginData(ORIGINS.CA, "/whats-new?populate=whats_new_news&populate=whats_new_news.image"),
    ...
    data.homepage_our_digital_solution_buttons.forEach((ds) => {
      if (ds.main_button_label) {
        digitalSolutionsTabContent[ds.main_button_label] = {
          ...
          learnMoreLink: ORIGINS.CA + ds.button_link,
          list: ds.homepage_our_digital_solution_pages_links.map((item) => ({
            link: ORIGINS.CA + item.link,
            label: item.label,
          })),
        };
      }
    ...
    <>
      ...
      {header && (
        <Banner
          ...
          ourWorkLink={ORIGINS.CA + header.buttons[1].link}
          button_link={ORIGINS.CA + header.buttons[0].link}
     ...
     {brandReputations && (
      ...
      {scheduleCall && (
        <div className="postImage">
          <HeroSection
            ...
            buttonLink={ORIGINS.CA + scheduleCall.button_link}
          />
        </div>
      )}
      ...
      {getStart && (
        <GetStartedSection
          ...
          buttonLink={ORIGINS.CA + getStart.button_link}
