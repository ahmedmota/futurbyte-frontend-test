...
export async function generateMetadata({ params: { id } }) {
  ...
  const data = await getOriginData(ORIGINS.CA,`/seos?filters[slug][$eq]=${id}`);
...
export default async function BlogDetailPage({ params }) {
  ...
  const getContentApi = await getOriginData(ORIGINS.CA,
    `/blog-detail-blogs?populate=*&filters[slug][$eq]=${id}`
  );
  const getContentFaqs = await getOriginData(ORIGINS.CA,
  ...
  const getRelatedBlogsApi = await getOriginData(ORIGINS.CA,
  ...
  const getMayLikeBlogsApi = await getOriginData(ORIGINS.CA,
