...
export default async function Services() {
  ...
  const bannerResponse = await getOriginData(ORIGINS.CA,"/services-header?populate=*");
  const servicesResponse = await getOriginData(ORIGINS.CA,
    "/services-sections?populate=image&populate=services_services_section_links"
  );
  const letsGetStartedResponse = await getOriginData(ORIGINS.CA,"/let-s-get-started?populate=*");
  const faqResponse = await getOriginData(ORIGINS.CA,"/frequently-ask-question?populate=*");
  const privileged = await getOriginData(ORIGINS.CA,"/privileged-to-be?populate=logos.logo");
  const header = await getOriginData(ORIGINS.CA,
